...
export default function CidadePage() {
  ...
  useEffect(() => {
    const fetchMontadores = async () => {
      ...
      try {
        const query = `
          *[_type == "montador" && estado == $uf && cidade match $city && status == "ATIVO"] {
            _id,
            "id": slug.current,
        ...
        console.error("Failed to fetch montadores:", error);
      } finally {
        setLoading(false);
      }
    };
    if (uf && formattedCity) fetchMontadores();
  }, [uf, formattedCity]);
    ...
    <div className="max-w-7xl mx-auto py-12 px-4 sm:px-6 lg:px-8">
      <div className="mb-12 border-b pb-8">
        <h1 className="text-4xl font-extrabold text-gray-900 mb-4">
          Montadores em {formattedCity}, {uf?.toUpperCase()}
        </h1>
        <p className="text-xl text-gray-600">
          Profissionais disponíveis para atendimento na sua região.
        </p>
      </div>
            ...
            <CardContent className="p-0">
              <div className="p-6">
                <div className="flex items-start gap-4 mb-4">
                  ...
                  <div>
                    <h3 className="font-bold text-lg text-gray-900 leading-tight mb-1">{montador.nome}</h3>
